...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
	...
	if (client->getCharacterCount(zoneServer.get()->getGalaxyID()) >= 32) {
		ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are limited to 32 characters per galaxy.", 0x0);
